...
//        Action action = actions.dragAndDrop(oslo,norway).build(); //drags and drops the sourse to the target
//        action.perform();

        Action action = actions.clickAndHold(oslo).moveToElement(norway).release().build();
        //clicks on the element and holds it, moves it to target then releases it.
        action.perform();
